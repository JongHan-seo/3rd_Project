...
void setup()
{
  ...
  pinMode(BLUE, OUTPUT);
  //자동운전은 파란색
  //수동운전은 off상태였다가 플래시 버튼 누르면 하얀색 빛이 나오게.
...
void loop()
{
  ...
  compare();//입력된 데이터에 따라 모터에 입력될 변수를 조정하는 함수
...
void rc_ctrl_val(unsigned char cmd) //입력된 데이터에 따라 모터에 입력될 변수를 조정하는 함수
{
  ...
  else if (cmd == 'l') //'a'가 입력되었을 때, 제자리 좌회전
  {
    m_a_dir = 1;  //모터A 역방향
    m_b_dir = 0;  //모터B 정방향
    m_a_spd = MOTOR_SPEED;  //모터A의 속력값 조정
  ...
  else if (cmd == 'r') //'d'가 입력되었을 때, 제자리 우회전
  {
    m_a_dir = 0;  //모터A 정방향
    m_b_dir = 1;  //모터B 역방향
    m_a_spd = MOTOR_SPEED;  //모터A의 속력값 조정
